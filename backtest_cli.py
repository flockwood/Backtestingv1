...
def backtest(symbol, strategy, short, long, start_date, end_date, capital, 
    ...
    Examples:
        ...
        # Simple backtest
        python3 backtest_cli.py -s AAPL
        
        # Custom parameters
        python3 backtest_cli.py -s MSFT --short 5 --long 15 --capital 25000
        
        # Generate report
        python3 backtest_cli.py -s GOOGL --report -o googl_report.html
        
        # Compare strategies
        python3 backtest_cli.py -s AAPL --compare
        
        # Optimize parameters
        python3 backtest_cli.py -s TSLA --optimize
    """
...
def run_single_backtest(symbol, strategy, short, long, start_date, end_date,
    ...
    try:
        ...
        if verbose:
            ...
            click.echo(f"  Return: {click.style(f'{result.total_return:.2%}', fg=return_color)}")
    ...
    if results:
        ...
        for r in results:
            ...
            click.echo(f"{r['name']:<15} {r['params']:<10} "
                      f"{click.style(return_str, fg=return_color):>10} "
                      f"{sharpe_str:>10} {dd_str:>10} {r['trades']:>10}")
...
def run_optimization(symbol, capital, start_date, end_date, data_source):
    ...
    if best_params:
        ...
        for i, r in enumerate(sorted_results, 1):
            return_str = f"{r['return']:.2%}"
            sharpe_str = f"{r['sharpe']:.2f}"
            click.echo(f"  {i}. SMA({r['short']}/{r['long']}): Sharpe={sharpe_str}, Return={return_str}")
